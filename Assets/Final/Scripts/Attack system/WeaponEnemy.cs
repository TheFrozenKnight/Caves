...
public class WeaponEnemy : MonoBehaviour
{
	...
	private void OnTriggerEnter(Collider other)
	{
        if (other.tag == "Player")
        {
            DmgInfo = new DmgInfo(dmgValue, dmgColor, other.transform.position);
            other.GetComponent<PlayerController>().ApplyDMG(other.transform.position - transform.position, 250f, DmgInfo);
        }
	}
